...
from dataset.LRS2_datasets import lrs2_datasets
...
def train(config):
    ...
    train_data_loader = DataLoader(lrs2_datasets(config.preprocessed_dir, 'train', window_size=config.window_size),
        batch_size=config.batch_size,
        num_workers=config.num_thread,
        shuffle=False, drop_last=True,
        
        )
    val_data_loader = DataLoader(lrs2_datasets(config.preprocessed_dir, "val", window_size=config.window_size), 
            batch_size=config.batch_size,
            num_workers=config.num_thread,
            shuffle=False, drop_last=True
            )
    ...
    for epoch in range(start_epoch, config.niter + config.niter_decay + 1):
        ...
        for step, (single_img, input_mfcc, img_seq) in tqdm(enumerate(train_data_loader)):
            ...
            if step % config.eval_interval == 0:
                ...
                with torch.no_grad():
                    eval_model(val_data_loader, step, Generator, epoch)
    
        update_learning_rate(net_g_scheduler, optimizer_G)
        update_learning_rate(net_d_scheduler, optimizer_D)
        

def eval_model(val_data_loader, step, model, epoch):
    eval_steps = 500
    print("Evaluating for {} steps".format(eval_steps))
    l1_losses = []
    step = 0
    
    for step, (single, input_mfcc, seq) in tqdm(enumerate(val_data_loader)):
        step += 1
        model.eval()
